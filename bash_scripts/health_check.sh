...
# Check Web Server Status (Nginx or Apache)
WEB_SERVER=""
...
# Logging
{
  ...
  if [ "$DISK_USAGE" -gt 90 ]; then
    echo "[$TIMESTAMP] ⚠️ WARNING: Disk space critically low (${DISK_USAGE}%)"
  fi

  if [ "$WEB_STATUS" != "active" ]; then
    echo "[$TIMESTAMP] ⚠️ WARNING: Web server ($WEB_SERVER) is not running"
  fi

  if [ "$API_1_STATUS" -ne 200 ]; then
    echo "[$TIMESTAMP] ⚠️ WARNING: /students API not responding correctly"
  fi

  if [ "$API_2_STATUS" -ne 200 ]; then
    echo "[$TIMESTAMP] ⚠️ WARNING: /subjects API not responding correctly"
  fi
} >> "$LOG_FILE"
